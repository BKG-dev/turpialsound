...
version: "2.0"
last_updated: "2026-05-18T14:13:00-04:00"
...
**Fases A, B, C, D COMPLETAS. S-MP-01..S-MP-08 cerrados. Fase E (estabilizacion documental + release gate) en curso.**
...
### ?? PENDIENTE INMEDIATO (ESTADO REAL 2026-05-18)

- **QA manual integral buyer -> admin -> escrow -> release** (pendiente de ejecución controlada).
- **Cron T+7** pendiente de implementación/validación operativa.
- **Pasarelas automáticas** (Mercantil/Binance) continúan diferidas por decisión de producto.
- **No solapar zonas activas recientes de Manuel:** `components/marketplace/dashboard/*`, `lib/marketplace/notifications.ts`, `actions/marketplace/transactions.ts`.
...
## PENDIENTE PRODUCCION - Verificar fallback SQL de createMarketplaceTransaction
...
**Contexto:** Codex implemento createMarketplaceTransaction con doble camino (Prisma nativo + SQL directo raw) para manejar columnas que podian faltar en DB. Las columnas orderId, quantity, unitPrice y la tabla mp_orders YA existen en produccion (migracion 20260517 aplicada). El fallback SQL es codigo defensivo (~150 lineas) que probablemente nunca se ejecuta. getMarketplaceSchemaCapabilities hace una query extra por cada transaccion.

**Tarea:** Al salir a produccion real, verificar que el camino Prisma nativo funciona sin fallback. Si es asi, eliminar createMarketplaceTransaction, getMarketplaceSchemaCapabilities, CreateMarketplaceTransactionInput, y createLegacyTransactionId de actions/marketplace/transactions.ts.
