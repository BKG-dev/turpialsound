...
function isQuerySecretFallbackEnabled(): boolean {
  return process.env.EVOLUTION_WEBHOOK_ALLOW_QUERY_SECRET?.trim().toLowerCase() === 'true'
}
...
export async function POST(request: NextRequest) {
  ...
  const allowQuerySecret = isQuerySecretFallbackEnabled()
  const isAuthorizedByHeader = headerSecret === expectedSecret
  const isAuthorizedByQuery = allowQuerySecret && querySecret === expectedSecret
