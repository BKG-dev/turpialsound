...
export class ReportBuilder {
  ...
  addCheck(moduleName, checkName, status, detail = null) {
    const mod = this.modules.find((m) => m.module === moduleName)
    if (!mod) {
      this.addModule(moduleName, 'RUNNING')
      return this.addCheck(moduleName, checkName, status, detail)
    }
    mod.checks.push({ check: checkName, status, detail })
    return this
  }
  ...
  writeJSON(filename = null) {
    const ts = this.runId
    const name = filename || `report-${ts}.json`
  ...
  writeMD(filename = null) {
    const ts = this.runId
    const name = filename || `report-${ts}.md`
    const dir = this.outputDir
    if (!existsSync(dir)) mkdirSync(dir, { recursive: true })
    const filePath = path.join(dir, name)
